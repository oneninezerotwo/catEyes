<template>
  <div class="nav-wrap filter-nav-wrap">
    <div class="tab mb-line-b">
      <div
        :class="{ item: true, chosenTitle: l_show == index }"
      ...
      <div class="tab-content">
        ...
        <div class="page special" v-show="l_show == 2">
          <div id="special-content" style="height: 386.4px;">
            ...
            <div class="item-list">
              <div
                :class="{ item: true, chosen: ac5 == index }"
                @click.stop="a_active_area4(index)"
                :data-id="k.id"
                data-type="service"
                v-for="(k, index) in service"
                :key="index"
                v-text="k.name"
              ></div>
            </div>
            ...
            <div class="item-list">
              <div
                :class="{ item: true, chosen: ac6 == index }"
                @click.stop="a_active_area5(index)"
                :data-id="k.id"
                data-type="hallType"
                v-for="(k, index) in hallType"
                :key="index"
                v-text="k.name"
              >
                全部
        ...
        <div class="page brand" v-show="l_show == 1">
          <div id="brand-content" style="height: 386.4px;">
            <div
              :class="{ item: true, 'brand-list': true, chosen: ac4 == index }"
              @click.stop="a_active_area3(index, k.name)"
              v-for="(k, index) in brands"
              :key="index"
              :data-id="k.id"
              data-type="brand"
            >
        ...
        <div class="page region" v-show="l_show == 0">
          <div id="region-tab">
            <ul class="tab">
              <li
                :class="{ item: true, chosen: s_tab_d_is1 }"
                @click.stop="s_tab_d1"
              >
                商区
              </li>
              <li
                :class="{ item: true, chosen: s_tab_d_is2 }"
                @click.stop="s_tab_d2"
              >
                地铁站
              </li>
          ...
          <div id="region-list" style="height: 450.8px;">
            <div id="region-sidenav">
              <div id="district" v-show="s_tab_d_is1">
                <div
                  ...
                  @click.stop="a_area(index)"
                  v-for="(k, index) in area"
                  :key="index"
                  data-type="district"
                  :data-id="k.id"
                  v-text="k.name + '(' + k.count + ')'"
                ></div>
              </div>
            ...
            <div id="region-list-item">
              <div id="filter-list">
                <div
                  :class="{ item: true, chosen: ac3 == index }"
                  @click.stop="a_active_area2(index, k.name)"
                  v-for="(k, index) in subContent['subItems']"
                  :key="index"
                  data-type="area"
                  :data-id="k.id"
                >
